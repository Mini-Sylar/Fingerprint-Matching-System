...
class SIFT:
    def __init__(self, float_tolerance=1e-7):
    ...
    def computeKeypointsAndDescriptors(self, image, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
        ...
        self.gaussian_images = self.generateGaussianImages(base_image, num_octaves, gaussian_kernels)
        self.dog_images = self.generateDoGImages(self.gaussian_images)
        keypoints = self.findScaleSpaceExtrema(self.gaussian_images, self.dog_images, num_intervals, sigma,
                                               image_border_width)
        keypoints = self.removeDuplicateKeypoints(keypoints)
        keypoints = self.convertKeypointsToInputImageSize(keypoints)
        descriptors = self.generateDescriptors(keypoints, self.gaussian_images)
        ...
        for gaussian_images_in_octave in gaussian_images:
            ...
            for first_image, second_image in zip(gaussian_images_in_octave, gaussian_images_in_octave[1:]):
                dog_images_in_octave.append(cv2.subtract(second_image,
                                                         first_image))  # ordinary subtraction will not work because the
                # images are unsigned integers
    ...
    def localizeExtremumViaQuadraticFit(self, i, j, image_index, octave_index, num_intervals, dog_images_in_octave,
                                        sigma,
        ...
        for peak_index in orientation_peaks:
            ...
            if peak_value >= peak_ratio * orientation_max:
                ...
                if abs(orientation - 360.) < self.float_tolerance:
                    orientation = 0
                new_keypoint = cv2.KeyPoint(*keypoint.pt, keypoint.size, orientation, keypoint.response,
                                            keypoint.octave)
    ...
    def generateDescriptors(self, keypoints, gaussian_images, window_width=4, num_bins=8, scale_multiplier=3,
        ...
        for keypoint in keypoints:
            ...
            half_width = int(
                round(hist_width * sqrt(2) * (
                        window_width + 1) * 0.5))  # sqrt(2) corresponds to diagonal length of a pixel
            half_width = int(
                min(half_width, sqrt(num_rows ** 2 + num_cols ** 2)))  # ensure half_width lies within image
